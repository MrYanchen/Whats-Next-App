                    ...
                    <div class="">
                        <ul class="slides">
                            <c:forEach items="${stepList}" var="step">
                              <c:set var="id" value="${step.getStepNumber()}"></c:set>
                              <input type="radio" name="radio-btn" id="img-${id}" <c:if test="${step.getStepNumber()==1}">checked</c:if> />
                              
                              <li class="slide-container">
                                <div class="slide">
                                    <img class="img-responsive" src="<c:url value="/item/${itemId}/step/${step.getStepId()}/image" />" height="300" width="320" />
                                    <div class="panel panel-default">
                                        <div class="panel-body">
                                            <div class="row">
                                                <div class="col-sm-2"><c:out value="${step.getStepNumber()}"/>/<c:out value="${stepList.size()}"/></div>
                                                <div class="col-sm-10"><c:out value="${step.getStepText()}"/></div>
                                            </div>
                                        </div>
                                    </div>
                                 </div>
                                 <div class="nav">
                                    <c:if test="${step.getStepNumber() == 1 && stepList.size() > 2}">
                                        <label for="img-${stepList.size()}" class="prev">&#x2039;</label>
                                        <label for="img-${step.getStepNumber()+1}" class="next">&#x203a;</label>
                                    </c:if>
                                    <c:if test="${step.getStepNumber() == stepList.size() && stepList.size() > 2}">
                                        <label for="img-${step.getStepNumber()-1}" class="prev">&#x2039;</label>
                                        <label for="img-1" class="next">&#x203a;</label>
                                    </c:if>
                                    <c:if test="${step.getStepNumber() > 1 && step.getStepNumber() < stepList.size()}">
                                        <label for="img-${step.getStepNumber()-1}" class="prev">&#x2039;</label>
                                        <label for="img-${step.getStepNumber()+1}" class="next">&#x203a;</label>
                                    </c:if>
                                    <c:if test="${stepList.size() == 2 && step.getStepNumber() == 1 }">
                                        <label for="img-${step.getStepNumber()+1}" class="prev">&#x2039;</label>
                                        <label for="img-${step.getStepNumber()+1}" class="next">&#x203a;</label>
                                    </c:if>
                                    <c:if test="${stepList.size() == 2 && step.getStepNumber() == 2 }">
                                        <label for="img-${step.getStepNumber()-1}" class="prev">&#x2039;</label>
                                        <label for="img-${step.getStepNumber()-1}" class="next">&#x203a;</label>
                                    </c:if>
                                 </div>
                           ...
                           <li class="nav-dots">
                              <c:forEach items="${stepList}" var="step">
                              <c:set var="id" value="${step.getStepNumber()}"></c:set>
                              <label for="img-${id}" class="nav-dot" id="img-dot-${id}"></label>
                              </c:forEach>
                           </li>
                        </ul>
                    </div>
